...
def find_centroid(polygon):
    ...
    a, cx, cy, i = 0, 0, 0, 0
    while i < n:
        v = polygon[i][0] * polygon[i + 1][1] - polygon[i][1] * polygon[i + 1][0]
        a += v
        cx += v * (polygon[i][0] + polygon[i + 1][0])
        cy += v * (polygon[i][1] + polygon[i + 1][1])
        i += 1
    a = a / 2
    if a == 0:
...
def find_center(polygons):
    ...
    -156.21763
    """
    i, Cx, Cy, Area, n = 0, 0, 0, 0, len(polygons)
      
    while i < n:
        Cenx, Ceny, a = find_centroid(polygons[i])
    
        Area += a
        Cx += Cenx * a
        Cy += Ceny * a
        i += 1
    average = (Cx / Area), (Cy / Area)
    return average 
...
def group_tweets_by_state(tweets):
    ...
    for i in range(len(tweets)):
        closest_state = find_closest_state(tweets[i], us_centers)
        if closest_state not in tweets_by_state.keys():
            tweets_by_state[closest_state] = list()   
        tweets_by_state[closest_state].append(tweets[i])
...
def most_talkative_state(term):
    ...
    states_with_term = group_tweets_by_state(tweets)
    
    most_talkative, state = 0, ''
    for k in states_with_term:
        talkative_state = len(states_with_term[k])
        if  most_talkative < talkative_state:
            most_talkative = talkative_state
            state = k
    return state
...
def average_sentiments(tweets_by_state):
    ...
    for k in tweets_by_state:
        counter = 0
        total = 0
        for i in range(0, len(tweets_by_state[k])):
            text = tweets_by_state[k][i]
            if has_sentiment (analyze_tweet_sentiment(text)) : 
                sentiment_value = analyze_tweet_sentiment(text)
                total += sentiment_value
                counter += 1
        if total not in (None, 0.0):
            averaged_state_sentiments[k] = total / counter
...
def group_tweets_by_hour(tweets):
    ...
    tweets_by_hour = {}
    for i in range(len(tweets)):
        hr1 = tweet_time(tweets[i])
        hr = hr1.hour
        if hr not in tweets_by_hour:
            tweets_by_hour[hr] = list()
        tweets_by_hour[hr].append(tweets[i])
